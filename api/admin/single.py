...
def list_all_results():
    try:
        ...
        today = date.today()
        new_results_today = sum(1 for user in results if user.created_at.date() == today)

        return {
            "bets": results_list,
            "total_bets": total_results,
            "new_bets_today": new_results_today,
        }
...
def list_transactions():
    try:
        ...
        transactions, withdrawals, deposits = list_all_transactions()
        ...
        new_withdrawals_today = sum(
            1 for withdrawal in withdrawals if withdrawal.created_at.date() == today
        )
        new_deposits_today = sum(
            1 for deposit in deposits if deposit.created_at.date() == today
        )
