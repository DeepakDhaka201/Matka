...
if(isset($_REQUEST['partner'])){
    ...
    $blog = query("select * from users where partner='$partner'  where created_at > $start_time AND created_at < $end_time");
} else {
    $blog = query("select * from users  where created_at > $start_time AND created_at < $end_time");
}
